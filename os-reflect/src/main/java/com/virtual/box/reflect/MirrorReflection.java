package com.virtual.box.reflect;
...
public final class MirrorReflection {
    ...
    public static <T> MirrorReflection with(Object obj){
        return on(obj.getClass());
    }
    ...
    private static void printStackTrace(Throwable e){
        Log.println(Log.ERROR, "Reflect.err",
                ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> START >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        e.printStackTrace();
        Log.println(Log.ERROR, "Reflect.err",
                ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> END >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
