...
class VmActiveServices {
    ...
    fun startService(service: Intent?, resolvedType: String?, requireForeground: Boolean, userId: Int): ComponentName? {
        ...
        val runningServiceRecord = getOrCreateRunningServiceRecord(service)
        runningServiceRecord.serviceInfo = serviceInfo
    ...
    fun stopService(intent: Intent?, resolvedType: String?, userId: Int): Int{
        ...
        try {
            processRecord.vmAppThread?.schduleStopService(intent)
    ...
    private fun createStubServiceIntent(
        ...
        val shadow = Intent()
        val shadowComponentName = ComponentName(VirtualBox.get().hostPkg,
    ...
    private fun getOrCreateRunningServiceRecord(intent: Intent): RunningServiceRecord {
        synchronized(recordLock){
            var runningServiceRecord = findRunningServiceRecord(intent)
            if (runningServiceRecord == null) {
                runningServiceRecord = RunningServiceRecord()
                runningServiceRecord.intent = intent
     ...
     */
    class RunningServiceRecord : Binder() {
