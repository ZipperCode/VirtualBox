...
public interface Parcelling<T> {
    ...
    class Cache {
        ...
        <P extends Parcelling<?>> P get(Class<P> clazz) {
    ...
    interface BuiltIn {
        ...
        class ForInternedString implements Parcelling<String> {
            ...
            public String unparcel(Parcel source) {
                return StringUtils.safeIntern(source.readString());
            }
        }
        ...
        class ForInternedStringArray implements Parcelling<String[]> {
            ...
            @Override
            public String[] unparcel(Parcel source) {
                String[] array = (String[]) source.readArray(String.class.getClassLoader());
                if (array != null) {
                    int size = array.length;
                    for (int index = 0; index < size; index++) {
                        array[index] = StringUtils.safeIntern(array[index]);
                    }
                }
                return array;
            }
        ...
        class ForInternedStringValueMap implements Parcelling<Map<String, String>> {
            ...
            public Map<String, String> unparcel(Parcel source) {
                ...
                for (int index = 0; index < map.size(); index++) {
                    map.setValueAt(index, StringUtils.safeIntern(map.valueAt(index)));
                }
                return map;
            }
        }
        ...
        class ForStringSet implements Parcelling<Set<String>> {
            ...
            public Set<String> unparcel(Parcel source) {
                ...
                Set<String> set = new CompatArraySet<>();
        ...
        class ForInternedStringSet implements Parcelling<Set<String>> {
            ...
            public Set<String> unparcel(Parcel source) {
                ...
                Set<String> set = new CompatArraySet<>();
                for (int count = 0; count < size; count++) {
                    set.add(StringUtils.safeIntern(source.readString()));
                }
                return set;
            }
        }
        ...
        class ForInternedStringArraySet implements Parcelling<CompatArraySet<String>> {

            @Override
            public void parcel(CompatArraySet<String> item, Parcel dest, int parcelFlags) {
                if (item == null) {
                    dest.writeInt(-1);
                } else {
                    dest.writeInt(item.size());
            ...
            @Override
            public CompatArraySet<String> unparcel(Parcel source) {
                final int size = source.readInt();
                if (size < 0) {
                  return null;
                }
                CompatArraySet<String> set = new CompatArraySet<>();
                for (int count = 0; count < size; count++) {
                    set.add(StringUtils.safeIntern(source.readString()));
                }
                return set;
