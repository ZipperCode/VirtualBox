...
public class ComponentResolver{
     ...
     */
    private static final Set<String> PROTECTED_ACTIONS = new CompatArraySet<>();
    static {
...
//        }
    ...
    void fixProtectedFilterPriorities() {
        synchronized (mLock) {
            ...
            for (int i = protectedFilters.size() - 1; i >= 0; --i) {
                ...
                if (DEBUG_FILTERS) {
                    Log.i(TAG, "Protected action; cap priority to 0;"
                            + " package: " + packageName
                            + " activity: " + className
    ...
    private void adjustPriority(List<ParsedActivity> systemActivities, ParsedActivity activity,
        ...
        if (!privilegedApp) {
            ...
            if (DEBUG_FILTERS) {
                Log.i(TAG, "Non-privileged app; cap priority to 0;"
                        + " package: " + packageName
                        + " activity: " + className
        ...
        if (isProtectedAction(intent)) {
            if (mDeferProtectedFilters) {
                ...
                if (DEBUG_FILTERS) {
                    Log.i(TAG, "Protected action; save for later;"
                            + " package: " + packageName
                            + " activity: " + className
                            + " origPrio: " + intent.getPriority());
                }
        ...
        if (foundActivity == null) {
            ...
            if (DEBUG_FILTERS) {
                Log.i(TAG, "New activity; cap priority to 0;"
                        + " package: " + packageName
                        + " activity: " + className
                        + " origPrio: " + intent.getPriority());
            }
        ...
        if (actionsIterator != null) {
            ...
            if (intentListCopy.size() == 0) {
                ...
                if (DEBUG_FILTERS) {
                    Log.i(TAG, "Mismatched action; cap priority to 0;"
                            + " package: " + packageName
                            + " activity: " + className
                            + " origPrio: " + intent.getPriority());
                }
        ...
        if (categoriesIterator != null) {
            ...
            if (intentListCopy.size() == 0) {
                // no more intents to match; we're not equivalent
                if (DEBUG_FILTERS) {
                    Log.i(TAG, "Mismatched category; cap priority to 0;"
                            + " package: " + packageName
                            + " activity: " + className
                            + " origPrio: " + intent.getPriority());
                }
        ...
        if (schemesIterator != null) {
            ...
            if (intentListCopy.size() == 0) {
                // no more intents to match; we're not equivalent
                if (DEBUG_FILTERS) {
                    Log.i(TAG, "Mismatched scheme; cap priority to 0;"
                            + " package: " + packageName
                            + " activity: " + className
                            + " origPrio: " + intent.getPriority());
                }
        ...
        if (authoritiesIterator != null) {
            ...
            if (intentListCopy.size() == 0) {
                // no more intents to match; we're not equivalent
                if (DEBUG_FILTERS) {
                    Log.i(TAG, "Mismatched authority; cap priority to 0;"
                            + " package: " + packageName
                            + " activity: " + className
                            + " origPrio: " + intent.getPriority());
                }
        ...
        if (intent.getPriority() > cappedPriority) {
            if (DEBUG_FILTERS) {
                Log.i(TAG, "Found matching filter(s);"
            ...
            for (int i = filter.countMimeGroups() - 1; i >= 0; i--) {
                ...
                for (int typeIndex = mimeTypes.size() - 1; typeIndex >= 0; typeIndex--) {
                    ...
                    try {
                        ...
                        if (DEBUG) {
                            Log.w(TAG, "Malformed mime type: " + mimeType, e);
                        }
                    }
                }
            }
    ...
    private static final class ProviderIntentResolver
        ...
        void addProvider(ParsedProvider p) {
            if (mProviders.containsKey(p.getComponentName())) {
                Log.w(TAG, "Provider " + p.getComponentName() + " already defined; ignoring");
                return;
            }
